...
if __name__ == "__main__":
    ...
    annotation_categories = ['words']
    delays = cfg.speech_delays
    subjects = cfg.subjects_analysis_novel + cfg.subjects_analysis_familiar
    save_dir = cfg.dir_analysis_results + "2_alignment_speech/k_set_to_words/"
    ROIs = ['low', 'high']
    ...
    for n_idx, n in enumerate(subjects):
        ...
        for i in [0,1]:
            ...
            plt.title('Subject ' + subject + ", words")
        ...
        for ROI in ROIs:
            ...
            boundaries = {}
            for block in blocks:
                n_words_changes = int(np.sum(annotations['words'][block]))
                ...
                bounds = GSBS_obj.get_strengths(n_words_changes + 1) # +1 because parameter is the number of states

                # Convert to deltas
                boundaries[block] = (bounds > 0).astype(int)
